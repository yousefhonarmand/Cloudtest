...
function updateCharts(servers) {
  ...
  if (!cpuChart) {
    ...
    cpuChart = new Chart(ctxCPU, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'CPU %', data: cpuData, backgroundColor: '#00ffcc' }] }
    });
  } else { cpuChart.data.datasets[0].data = cpuData; cpuChart.update(); }

  if (!ramChart) {
    const ctxRAM = document.getElementById('chartRAM').getContext('2d');
    ramChart = new Chart(ctxRAM, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'RAM %', data: ramData, backgroundColor: '#ff6600' }] }
    });
  } else { ramChart.data.datasets[0].data = ramData; ramChart.update(); }
}
